...
def test_sample_empirical_pdf_shapes(
    x: np.ndarray, pdf: np.ndarray, shape: Optional[Union[Tuple[int], int]]
) -> None:
    y = sample_empirical_pdf(x, pdf, shape, tol=1e-5)
    expected_shape = (
        () if shape is None else ((shape,) if isinstance(shape, int) else shape)
    )
