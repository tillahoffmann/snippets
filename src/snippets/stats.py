...
def evaluate_bounded_kde_logpdf(
    kde: gaussian_kde, X: np.ndarray, bounds: np.ndarray
) -> np.ndarray:
    """
    ...
    # Handle sample shapes just like scipy.stats.gaussian_kde does (see source of
    # `logpdf`).
    X = np.atleast_2d(X)
...
class GaussianKernelDensity(BaseEstimator):
    ...
    The implementation employs :class:`~scipy.stats.gaussian_kde` to facilitate more
    sophisticated kernel covariances than supported by
    :class:`~sklearn.neighbors.KernelDensity` which only admits isotropic kernels.

    Args:
        bandwidth: Bandwidth selection method or scalar factor (see
            :class:`~scipy.stats.gaussian_kde` for details).
        bounds: Array of lower and upper bounds for each dimension (use :code:`nan` for
            unbounded or semi-bounded domains). Test samples are reflected at the
            supplied boundaries as part of :meth:`score_samples` to account for
            probability mass that has "leaked out of" the support (see
            `Boneva et al. (1971)
            <https://doi.org/10.1111/j.2517-6161.1971.tb00855.x>`__ for details).
    ...
    def __init__(
        ...
        if self.bounds is not None:
            ...
            assert self.bounds.ndim == 2 and self.bounds.shape[1] == 2, (
                f"Expected shape `(n_features, 2)` for `bounds`; got "
                f"{self.bounds.shape}."
            )

    def fit(
        self, X: np.ndarray, y: None = None, sample_weight: Optional[np.ndarray] = None
    ) -> GaussianKernelDensity:
        X = check_array(X)
        assert self.bounds is None or X.shape[1] == self.bounds.shape[0], (
            f"Bounds have {self.bounds.shape[0]} dimensions, but data has {X.shape[1]} "
            "dimensions."
        )
    ...
    def score_samples(self, X: np.ndarray) -> np.ndarray:
        ...
        assert X.shape[1] == self.n_features_in_, (
            f"Estimator was fit to {self.n_features_in_} features but samples have "
            f"{X.shape[1]}."
        )
